...
def fcstValue_fix_SNO(sno: str) -> float:
    ...
    match = re.match(r'(\d+\.\d+)cm', sno) # '실수cm' 형태의 문자열이 감지되면 실수부 추출
    if match:
        replace = float(match.group(1))
...
def main(args_tuple: tuple = None):
    ...
    # env_state
    parser_env_state = subparser.add_parser('env_state', add_help=True)
    parser_env_state.add_argument('--pointer', action='store', type=int)
    # weather_state
    parser_weather_state = subparser.add_parser('weather_state', add_help=True)
    parser_weather_state.add_argument('--pointer', action='store', type=int)

    # past_weather_state
    past_weather_state = subparser.add_parser('past_weather_state', add_help=True)
    past_weather_state.add_argument('--pointer', action='store', type=str, required=True)

    # predict
    parser_predict = subparser.add_parser('predict', add_help=True)
    parser_predict.add_argument('--location_grid', nargs=2, type=str, required=True)
    #parser_predict.add_argument('--guide', action='store_true', default=False)
    ...
    if args.mode == 'env_state':
        if args.pointer:
            result.append(pointed_data_load(STN, API_KEY, args.pointer, args.timeout))
        else:
            result.append(now_data_load(STN, API_KEY, args.timeout))

    elif args.mode == 'weather_state':
        if args.pointer:
            result.append(pointed_weather_load(STN, API_KEY, args.pointer, args.timeout))
        else:
            result.append(now_weather_load(STN, API_KEY, args.timeout))

    elif args.mode == 'past_weather_state':
        tmp = past_weather_load(STN, API_KEY, args.pointer, args.timeout)
        result.append(tmp[0])
        result.append(tmp[1])
        ...
        today= now_[:8]
        today_time = now_[8:]
        inp = [API_KEY2, today, today_time, args.location_grid[0], args.location_grid[1]]
        tmp = predict_data_load(inp, args.timeout)

        if len(tmp) != 5:
            raise Exception(f'{now}\n기상청 API 일기예보 정보 무결성 손상')

        result.append(predict_data_load(inp, args.timeout))
        
    else:
        d = ['env_state', 'weather_state', 'past_weather_state', 'predict']
        raise Exception(f'동작이 명확히 지정되지 않았습니다\n{d} 중 한가지 동작을 지정해 주십시오')

    if args_tuple == None:
        print(result[0]) ## datetime.datetime.now().strftime('%Y%m%d%H%M%S.%f') 
