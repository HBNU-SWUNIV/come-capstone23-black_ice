...
if __name__ == '__main__':
    try:
        ...
        # getimage
            ...
            print(f"{datetime.now().strftime('%Y%m%d%H%M%S.%f')}", result.get_transformed_image(), file=sys.stdout, flush=True)
            sys.exit(0)
    ...
    # 에러 발생 시 출력하고 종료
        ...
        print(f'{__file__}: {e}', file=sys.stderr, flush=True)
        sys.exit(1)
...
# python Sources/Main/Data_collection/Camera/roadcamera.py configure --port_cam 0 --img_save_name testimg --img_format png --perspective_after_size 300 300 --configure_save_name Sources/Main/TEST_RESOURCE/roadcamera.conf
