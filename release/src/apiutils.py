...
## 사용 지양
def weather_parse(info: str) -> np.ndarray:
    '''
    info = 다음과 같은 입력 자료
...
## 사용 지양
def past_aws2_weather_load(stn: str, api_key: str, end: str) -> np.ndarray:
    '''
    과거 4시간 이내의 현천 데이터를 리턴합니다. (5분 간격), 정보 누락 가능성 높음. 사용 지양.
    ...
    #  N   : range 기간 동안에 관측된 현상종류의 수
    #  WW1 : 현상번호
    #  NN1 : WW1 현상의 관측된 횟수(분)
    #  ... : WW1, NN1 이 N 갯수만큼 반복됨
    #--------------------------------------------------------------------------------------------------
    # 현천코드 : 0~2(맑음), 4(연무), 10(박무), 30(안개), 40~42(비), 50~59(안개비), 60~68(비), 71~76(눈)
    #--------------------------------------------------------------------------------------------------
    # YYMMDDHHMI,  STN,         LON,         LAT, S, N, WW1, NN1, ...,=
    #        KST,   ID,       (deg),       (deg),  ,  ,    ,    , ...,=
    202304141200,  548,127.63955688, 37.26882935, 2, 2,   0, 238,   4,   2,=
    202304141210,  548,127.63955688, 37.26882935, 2, 2,   0, 238,   4,   2,=
    202304141220,  548,127.63955688, 37.26882935, 2, 2,   0, 238,   4,   2,=
    202304141230,  548,127.63955688, 37.26882935, 2, 2,   0, 238,   4,   2,=
    202304141240,  548,127.63955688, 37.26882935, 2, 2,   0, 238,   4,   2,=
    #7777END
    ...
    url = f'https://apihub.kma.go.kr/api/typ01/cgi-bin/url/nph-aws2_min_ww1?stn={stn}&tm1={start}&tm2={end}&authKey={api_key}'
    try:
        res = requests.get(url)
    except:
        return ['Exception ', 'Fail Connection']
